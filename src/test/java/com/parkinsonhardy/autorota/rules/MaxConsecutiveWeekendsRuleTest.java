...
public class MaxConsecutiveWeekendsRuleTest extends RotaEngineTestBase {
    ...
    public void testMaxConsecutiveWeekendRulePasses2() throws RotaException {
        ...
        rotaEngine.assignShifts(saturdayInTwoWeeks.plusDays(1), saturdayInTwoWeeks.plusWeeks(1).plusDays(1));

        checkSingleEmployeeHasShiftCount(rotaEngine.getEmployees(), 9);
    }
    ...
    @Test(expected = RotaException.class)
    public void testMaxConsecutiveWeekendRuleThrowsAddedInMiddle() throws RotaException {
        ShiftDefinition dayShift = new ShiftDefinition("Day", LocalTime.parse("10:30"), LocalTime.parse("15:30"));
        rotaEngine.addShiftDefinition(dayShift);
        addSingleEmployee();
        addShiftRequirementForEveryDay(rotaEngine, "Day", 1);
        rotaEngine.addRule(new MaxConsecutiveWeekendsRule(2));

        DateTime saturday = getNextMonday().plusDays(5);
        DateTime saturdayInTwoWeeks = saturday.plusWeeks(1);
        DateTime saturdayInThreeWeeks = saturdayInTwoWeeks.plusWeeks(1);

        addShiftToSingleEmployee(dayShift, saturday);
        addShiftToSingleEmployee(dayShift, saturdayInThreeWeeks);

        rotaEngine.assignShifts(saturdayInTwoWeeks, saturdayInTwoWeeks.plusDays(2));
    }

    @Test
    public void testMaxConsecutiveWeekendRuleDoesntThrowAddedInMiddle() throws RotaException {
        ShiftDefinition dayShift = new ShiftDefinition("Day", LocalTime.parse("10:30"), LocalTime.parse("15:30"));
        rotaEngine.addShiftDefinition(dayShift);
        addSingleEmployee();
        addShiftRequirementForEveryDay(rotaEngine, "Day", 1);
