...
public class MinHoursAfterConsecutiveShiftsRuleTest extends RotaEngineTestBase {
    ...
    @Test(expected = RotaException.class)
    public void testMinHoursAfterShiftRuleThrowsShiftInPast() throws RotaException {
        ...
        rotaEngine.assignShifts(monday, tuesday);
    }

    @Test(expected = RotaException.class)
    public void testMinHoursAfterShiftRuleThrowsShiftInPast2() throws RotaException {
        ShiftDefinition dayShift = new ShiftDefinition("Day", LocalTime.parse("10:30"), LocalTime.parse("15:30"));
        rotaEngine.addShiftDefinition(dayShift);
        addSingleEmployee();
        addShiftRequirementForEveryDay(rotaEngine, "Day", 1);
        rotaEngine.addRule(new MinHoursAfterConsecutiveShiftsRule("Day", 2, 50));
        ...
        addShiftToSingleEmployee(dayShift, tuesday);
        addShiftToSingleEmployee(dayShift, wednesday);

        rotaEngine.assignShifts(monday, tuesday);
    }

    @Test(expected = RotaException.class)
    public void testMinHoursAfterShiftRuleThrowsShiftInMiddle() throws RotaException {
        ShiftDefinition dayShift = new ShiftDefinition("Day", LocalTime.parse("10:30"), LocalTime.parse("15:30"));
        rotaEngine.addShiftDefinition(dayShift);
        addSingleEmployee();
        addShiftRequirementForEveryDay(rotaEngine, "Day", 1);
        ...
        rotaEngine.assignShifts(tuesday, wednesday);
    }
}
